...
MONGO_URI = "mongodb+srv://mrheartake:190122@cluster0.t3lnuma.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
DB_NAME = "Laptop"
...
def delete_product(product_id):
    print(f"Deleting product with ID: {product_id}")  # ใช้สำหรับ debug การลบสินค้า
    result = collection.delete_one({"_id": int(product_id)})
    if result.deleted_count > 0:
        print("Product deleted successfully")  # ใช้สำหรับ debug การลบสินค้า
        products = list(collection.find())
        return jsonify({"message": "Product deleted successfully", "product": products}), 200
    else:
        print("Product not found")  # ใช้สำหรับ debug การลบสินค้า
        return jsonify({"error": "Product not found"}), 404
